    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            actual_num_epochs += 1
            for item in train_result.losses:
                item = float(item)
...
class RNNTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        # forward
        y_pred, h_s = self.model(x, self.hidden_state)
        # calculate loss
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        with torch.no_grad():
            ...
            #foward:
            y_scores,self.test_hidden_state = self.model(x,self.test_hidden_state)

            #loss:
            loss=self.loss_fn(y_scores.transpose(1,2),y)

            #num of correct:
            pred = torch.argmax(y_scores,dim=2)
...
class VAETrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # loss
        loss, data_loss, kldiv_loss = self.loss_fn(xr, x, mu, log_sigma2)
        # ========================
